...
class TextProcessor:
    ...
    def __init__(self):
        ...
        self.regex_patterns = {
            ...
            "CREDENTIAL_PASSWORD": r"\b(?:password|passwd|pwd|secret|api_key|access_token)\s*[:=is\s]+['\"]?[^\s'\"]+['\"]?",
            "CREDENTIAL_PIN": r"\b(?:bank\s+pin|card\s+pin|pin\s+code|atm\s+pin|pin)\s*[:=is\s]+['\"]?\d{4,8}['\"]?",
            "CREDENTIAL_OTP": r"\b(?:otp|one[- ]time[- ]password|verification\s+code)\s*[:=is\s]+['\"]?\d{4,8}['\"]?",
