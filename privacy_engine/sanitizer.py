...
PII_PATTERNS = [
    ...
    ("BANK_ACCOUNT_NUMBER", r'\b(?:(?:bank\s+)?account\s*(?:num|no|number)?|bank\s+acc|beneficiary\s+acc(?:ount)?|transfer\s+(?:money\s+|funds\s+)?to\s+(?:bank\s+)?account|payment\s+to\s+(?:bank\s+)?account|IFSC\s+[A-Z]{4}0[A-Z0-9]{6}\s+(?:and\s+)?account)\s*(?:is|to|:|=)?\s*(\d{9,18})\b', "[BANK_ACCOUNT_REDACTED]"),
    ...
    ("CREDENTIAL_PASSWORD", r'(?:\b(?:database|db|server|app|application|login|admin|account|root|user|api|client)?\s*(?:password|passwd|pwd))\s*[:=]\s*[\'"]?([^\s\'",;]{3,})[\'"]?|(?:\b(?:my|the|our|admin|user|root|account|db|database|server|app|application|login)?\s*(?:(?:database|db|server|app|application|login|admin|account|root|user)\s+)?(?:password|passwd|pwd)\s+is\s+[\'"]?([^\s\'",;]{3,})[\'"]?)', "[PASSWORD_REDACTED]"),
